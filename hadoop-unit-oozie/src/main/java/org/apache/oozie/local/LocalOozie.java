...
/**
 * LocalOozie runs workflows in an embedded Oozie instance . LocalOozie is meant for development/debugging purposes
 * only.
 */
public class LocalOozie {
     ...
     * @throws Exception if LocalOozie could not be started.
     */
    public synchronized static void start(String host, int port) throws Exception {
        if (localOozieActive) {
        ...
        container.addServletEndpoint("/versions", VersionServlet.class);
        container.addServletEndpoint("/v0/admin/*", V0AdminServlet.class);
        container.addServletEndpoint("/v1/admin/*", V1AdminServlet.class);
        container.addServletEndpoint("/v2/admin/*", V2AdminServlet.class);
        container.addServletEndpoint("/v0/jobs", V0JobsServlet.class);
...
//        container.addServletEndpoint("/v2/validate", V2ValidateServlet.class);
        if (port != 0) {
    ...
    /**
     * Return a {@link org.apache.oozie.client.OozieClient} for LocalOozie. The returned instance is configured
