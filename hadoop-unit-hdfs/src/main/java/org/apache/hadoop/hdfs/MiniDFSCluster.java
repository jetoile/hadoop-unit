...
public class MiniDFSCluster {
    ...
    static {
        try {
            PropertiesConfiguration propertiesConfiguration = new PropertiesConfiguration(HadoopUnitConfig.DEFAULT_PROPS_FILE);
            nameNodeHost = propertiesConfiguration.getString(HadoopUnitConfig.HDFS_NAMENODE_HOST_KEY);
    ...
    private void createNameNodesAndSetConf(MiniDFSNNTopology nnTopology,
        ...
        if (!federation && nnTopology.countNameNodes() == 1) {
            ...
            // NN is started.
            conf.set(FS_DEFAULT_NAME_KEY, "hdfs://" + nameNodeHost + ":" + onlyNN.getIpcPort());
        }
    ...
    private static void initNameNodeAddress(Configuration conf,
        ...
        String key = DFSUtil.addKeySuffixes(
                ...
                nnConf.getNnId());
        conf.set(key, nameNodeHost + ":" + nnConf.getHttpPort());

        key = DFSUtil.addKeySuffixes(
                DFS_NAMENODE_RPC_ADDRESS_KEY, nameserviceId,
                nnConf.getNnId());
        conf.set(key, nameNodeHost + ":" + nnConf.getIpcPort());
    }
    ...
    public synchronized void startDataNodes(Configuration conf, int numDataNodes,
        ...
        if (checkDataNodeHostConfig) {
            conf.setIfUnset(DFS_DATANODE_HOST_NAME_KEY, nameNodeHost);
        } else {
            conf.set(DFS_DATANODE_HOST_NAME_KEY, nameNodeHost);
        }
     ...
     * @throws IOException if not simulatedFSDataset
    ...
    protected void setupDatanodeAddress(Configuration conf, boolean setupHostsFile,
        ...
        if (setupHostsFile) {
            ...
            String address = nameNodeHost + ":" + NetUtils.getFreeSocketPort();
            ...
            if (checkDataNodeAddrConfig) {
                conf.setIfUnset(DFS_DATANODE_ADDRESS_KEY, nameNodeHost + ":0");
            } else {
                conf.set(DFS_DATANODE_ADDRESS_KEY, nameNodeHost + ":0");
            }
        }
        if (checkDataNodeAddrConfig) {
            conf.setIfUnset(DFS_DATANODE_HTTP_ADDRESS_KEY, nameNodeHost + ":0");
            conf.setIfUnset(DFS_DATANODE_IPC_ADDRESS_KEY, nameNodeHost + ":0");
        } else {
            conf.set(DFS_DATANODE_HTTP_ADDRESS_KEY, nameNodeHost + ":0");
            conf.set(DFS_DATANODE_IPC_ADDRESS_KEY, nameNodeHost + ":0");
        }
    }
