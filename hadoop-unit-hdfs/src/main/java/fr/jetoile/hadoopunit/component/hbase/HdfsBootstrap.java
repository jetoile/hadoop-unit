 ...
 */
package fr.jetoile.hadoopunit.component.hbase;
...
import fr.jetoile.hadoopunit.component.hadoop.BootstrapHadoop;
...
public class HdfsBootstrap implements BootstrapHadoop {
    ...
    public void loadConfig(Map<String, String> configs) {
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_NAMENODE_HOST_KEY))) {
            host = configs.get(HadoopUnitConfig.HDFS_NAMENODE_HOST_KEY);
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_NAMENODE_PORT_KEY))) {
            port = Integer.parseInt(configs.get(HadoopUnitConfig.HDFS_NAMENODE_PORT_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_NAMENODE_HTTP_PORT_KEY))) {
            httpPort = Integer.parseInt(configs.get(HadoopUnitConfig.HDFS_NAMENODE_HTTP_PORT_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_TEMP_DIR_KEY))) {
            tempDirectory = configs.get(HadoopUnitConfig.HDFS_TEMP_DIR_KEY);
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_NUM_DATANODES_KEY))) {
            numDatanodes = Integer.parseInt(configs.get(HadoopUnitConfig.HDFS_NUM_DATANODES_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_ENABLE_PERMISSIONS_KEY))) {
            enablePermission = Boolean.parseBoolean(configs.get(HadoopUnitConfig.HDFS_ENABLE_PERMISSIONS_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_FORMAT_KEY))) {
            format = Boolean.parseBoolean(configs.get(HadoopUnitConfig.HDFS_FORMAT_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.HDFS_ENABLE_RUNNING_USER_AS_PROXY_USER))) {
