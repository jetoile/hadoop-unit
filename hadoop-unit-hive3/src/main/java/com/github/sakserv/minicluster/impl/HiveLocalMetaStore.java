...
public class HiveLocalMetaStore implements MiniCluster {
    ...
    private static class StartHiveLocalMetaStore implements Runnable {
        ...
        public void run() {
            try {
                MetaStoreTestUtils.startMetaStore(hiveMetastorePort, HadoopThriftAuthBridge.getBridge(), hiveConf);
...
//        hiveConf.setVar(HiveConf.ConfVars.METASTOREURIS,"thrift://" + hiveMetastoreHostname + ":" + hiveMetastorePort);
//        hiveConf.setVar(HiveConf.ConfVars.SCRATCHDIR, hiveScratchDir);
        hiveConf.setVar(HiveConf.ConfVars.METASTORECONNECTURLKEY, "jdbc:derby:;databaseName=" + hiveMetastoreDerbyDbDir + ";create=true");
