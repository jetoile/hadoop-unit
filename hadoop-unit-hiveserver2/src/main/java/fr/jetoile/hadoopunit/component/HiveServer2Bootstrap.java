...
public class HiveServer2Bootstrap implements BootstrapHadoop {
    ...
    private void loadConfig() throws BootstrapException {
        host = configuration.getString(HiveServerConfig.HIVE_SERVER2_HOSTNAME_KEY);
        port = configuration.getInt(HiveServerConfig.HIVE_SERVER2_PORT_KEY);
        hostMetastore = configuration.getString(HiveServerConfig.HIVE_METASTORE_HOSTNAME_CLIENT_KEY);
        portMetastore = configuration.getInt(HiveServerConfig.HIVE_METASTORE_PORT_KEY);
        scratchDirectory = getTmpDirPath(configuration, HiveServerConfig.HIVE_SCRATCH_DIR_KEY);
        warehouseDirectory = configuration.getString(HiveServerConfig.HIVE_WAREHOUSE_DIR_KEY);
    ...
    public void loadConfig(Map<String, String> configs) {
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_SERVER2_HOSTNAME_KEY))) {
            host = configs.get(HiveServerConfig.HIVE_SERVER2_HOSTNAME_KEY);
        }
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_SERVER2_PORT_KEY))) {
            port = Integer.parseInt(configs.get(HiveServerConfig.HIVE_SERVER2_PORT_KEY));
        }
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_METASTORE_HOSTNAME_CLIENT_KEY))) {
            hostMetastore = configs.get(HiveServerConfig.HIVE_METASTORE_HOSTNAME_CLIENT_KEY);
        }
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_METASTORE_PORT_KEY))) {
            portMetastore = Integer.parseInt(configs.get(HiveServerConfig.HIVE_METASTORE_PORT_KEY));
        }
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_SCRATCH_DIR_KEY))) {
            scratchDirectory = getTmpDirPath(configs, HiveServerConfig.HIVE_SCRATCH_DIR_KEY);
        }
        if (StringUtils.isNotEmpty(configs.get(HiveServerConfig.HIVE_WAREHOUSE_DIR_KEY))) {
            warehouseDirectory = configs.get(HiveServerConfig.HIVE_WAREHOUSE_DIR_KEY);
        }
