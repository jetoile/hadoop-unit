 ...
 */
package fr.jetoile.hadoopunit.component.kafka;
...
public class KafkaBootstrap implements Bootstrap {
    ...
    public void loadConfig(Map<String, String> configs) {
        if (isNotEmpty(configs.get(HadoopUnitConfig.KAFKA_HOSTNAME_KEY))) {
            host = configs.get(HadoopUnitConfig.KAFKA_HOSTNAME_KEY);
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.KAFKA_PORT_KEY))) {
            port = Integer.parseInt(configs.get(HadoopUnitConfig.KAFKA_PORT_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.KAFKA_TEST_BROKER_ID_KEY))) {
            brokerId = Integer.parseInt(configs.get(HadoopUnitConfig.KAFKA_TEST_BROKER_ID_KEY));
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.KAFKA_TEST_TEMP_DIR_KEY))) {
            tmpDirectory = configs.get(HadoopUnitConfig.KAFKA_TEST_TEMP_DIR_KEY);
        }
        if (isNotEmpty(configs.get(HadoopUnitConfig.ZOOKEEPER_HOST_KEY)) && isNotEmpty(configs.get(HadoopUnitConfig.ZOOKEEPER_PORT_KEY))) {
